...
class CompanyViewSet(viewsets.ViewSet):
    ...
    def delete(self, request, pk=None):
        drinks = Company.objects.filter(pk=pk)
...
class EmployeeViewset(viewsets.ViewSet):
    ...
    permission_classes = [IsAuthenticated]
    def delete(self, request, pk=None):
        drinks = Employee.objects.filter(pk=pk)
        print("bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
        drinks.delete()
        return Response({"error":False,"message":"Data deleted successfully"})
...
class GenerateBillViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        serializer.is_valid()
        print(request.data['medicine_details'])
        for medicine in request.data['medicine_details']:
            medicine_single=Medicine.objects.get(id=medicine['id'])
            print(medicine_single)
            if medicine_single.in_stock_total < int(medicine['qty']):
                dict_response = {"error": True, "message": "Medicine Is Out Of Stock"}
                return Response(dict_response)
